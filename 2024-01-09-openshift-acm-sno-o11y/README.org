...
For edge scenarios we only send metrics to the hub cluster if certain thresholds are hit for a certain period of time (here 70% for more than 2 minutes - you can see this configuration in the open-cluster-management-addon-observability namespace under ConfigMaps observability-metrics-allowlist in the collect_rules section under SNOHighCPUUsage). Hence we now deploy cpu-heavy workload to hit those thresholds which in turn triggers the metrics being sent from sno to the ACM hub cluster.
